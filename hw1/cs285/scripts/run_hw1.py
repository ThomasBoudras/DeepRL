...
def run_training_loop(params):
    ...
    for itr in range(params['n_iter']):
        ...
        if itr == 0:
            ...
            paths, envsteps_this_batch = utils.sample_trajectories(env=env, policy=actor, min_timesteps_per_batch=params['batch_size'], max_path_length=params['ep_len'])
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
          ...
          idx_permu = np.random.permutation(range(len(replay_buffer.obs)))[:params["train_batch_size"]]
          ob_batch, ac_batch = replay_buffer.obs[idx_permu], replay_buffer.acs[idx_permu]
